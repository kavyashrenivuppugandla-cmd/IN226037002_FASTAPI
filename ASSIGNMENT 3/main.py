...
def discount(category:  str  = Query(None, description='Electronics or Stationery'),
             discount_percent: int = Query(..., gt=1, lt=99, description='Discount percentage to apply')):
    if not category:
    ...
    for product in products:
        if product['category'] == category:
            ...
            updated_products.append({'id': product['id'], 'name': product['name'], 'new_price': product['price']})

    if not updated_products:
        return {'message': f'No products found in category {category}'}

    return {'message': f'{len(updated_products)} products updated with {discount_percent}% discount in {category} category', 'updated_count': len(updated_products), 'updated_products': updated_products}
...
def inventory_summary():
    ...
    return {
        ...
        'out_of_stock': out_of_stock_names,
