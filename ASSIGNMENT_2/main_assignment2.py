...
def filter_products(category:str=Query(None), min_price:int=Query(None,description='Minimum price'),
...
class BulkOrder(BaseModel):
    ...
    contact_email: str = Field(...,min_length=2 )

@app.post('/orders/bulk')
def place_bulk_order(bulk_order: BulkOrder):
    confirmed=[]
    failed=[]
    grand_total = 0
    ...
    for item in bulk_order.items:
        ...
        if not product:
            ...
            grand_total += total
            confirmed.append({'product':product["name"],'quantity':item.quantity,'total_price':total})
    return {'company_name': bulk_order.company_name,'confirmed': confirmed, 'failed_orders': failed, 'grand_total': grand_total}     
 # ── Day 2: POST + Pydantic ─────────────────────────────────────────
...
def submit_feedback(feedback_data: CustomerFeedback):
    ...
    return {'message': 'Feedback submitted successfully', 'feedback': feedback_data.dict(),'total_feedback': len(feedback)}
...
class Order(BaseModel):
    customer_name: str
    delivery_address: str = Field(..., min_length=7)
    product_id: int
    quantity: int
orders = []
@app.post("/orders")
